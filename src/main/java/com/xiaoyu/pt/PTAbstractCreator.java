...
public abstract class PTAbstractCreator {
	...
	private void createJavaBean(String templateName, String suffix, String fileNameSuffix,
		...
		path.append("src/main/resources/pt/" + templateName);
	...
	public void createHtml(TableInfo tableinfo) {
		params.put("tableinfo", tableinfo);
		JavaBeanParam param = configBeanParam();
		createHtml(tableinfo.getFileName(), param.getTemplateName(), param.getSuffix(), param.getFileNameSuffix(), param.getPackageSuffix());
	}

	private void createHtml(String fileName, String templateName, String suffix, String fileNameSuffix, String packageSuffix) {
		TableInfo tableinfo = (TableInfo) params.get("tableinfo");
		VelocityEngine ve = new VelocityEngine();
		ve.setProperty("input.encoding", "utf-8");
		ve.setProperty("output.encoding", "utf-8");
		...
		//模板合并，得到期望文件
		t.merge(ctx, tmp);
		FileIOUtil.htmlCreateFile(fileName, tableinfo.getTableNameFt().toLowerCase() + fileNameSuffix + suffix,
				packageSuffix, tmp.toString());
	}

	public void createJs(TableInfo tableinfo) {
		params.put("tableinfo", tableinfo);
		JavaBeanParam param = configBeanParam();
		createJs(tableinfo.getFileName(), param.getTemplateName(), param.getSuffix(), param.getFileNameSuffix(), param.getPackageSuffix());
	}

	private void createJs(String fileName, String templateName, String suffix, String fileNameSuffix, String packageSuffix) {
		TableInfo tableinfo = (TableInfo) params.get("tableinfo");
		VelocityEngine ve = new VelocityEngine();
		ve.setProperty("input.encoding", "utf-8");
		ve.setProperty("output.encoding", "utf-8");
		...
		//模板合并，得到期望文件
		t.merge(ctx, tmp);
		FileIOUtil.jsCreateFile(fileName, tableinfo.getTableNameFt().toLowerCase() + fileNameSuffix + suffix,
				packageSuffix, tmp.toString());
	}

	public abstract PTAbstractCreator.JavaBeanParam configBeanParam();
